...
//debuge($resource);        
    ...
    ?><tr class="wpbc_row" id="resource_<?php echo $resource['id']; ?>"><?php
                ...
                <td style="text-align:center;">					
					<fieldset>
						<?php
						$resource['export'] = empty( $resource['export'] ) ? '' : $resource['export'];
						?>
						...
						<input type="text" 
								value="<?php echo esc_attr( $resource['export' ] ); ?>"
								id="booking_export_feed<?php echo $resource['id' ]; ?>" 
								name="booking_export_feed<?php echo $resource['id' ]; ?>" 
								class="large-text" 
								style="width:75%;" 
						 />
						<a href="<?php echo esc_url( trim( home_url(), '/' ) . '/' . trim( $resource['export' ], '/') );  ?>"
						   title="<?php _e( 'Open in new window', 'booking' ); ?>"
						   target="_blank"><i class="menu_icon icon-1x wpbc_icn_open_in_new"></i></a>
						<input
							name="url_full_<?php echo $resource['id' ]; ?>" id="url_full_<?php echo $resource['id' ]; ?>"
							class="put-in" type="text" readonly="readonly" onfocus="this.select()"
							value="<?php echo esc_attr( trim( home_url(), '/' ) . '/' . trim( $resource['export'], '/' ) ); ?>"
							/>
					</fieldset>
                </td>
