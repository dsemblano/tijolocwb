		...
		?><div class="control-group wpbc-no-padding" id="wpbc_save_simple_booking_form"><?php
			?><div class="btn-group" style="margin-top:7px;">
				<?php
					...
					$el_id = 'is_use_simple_booking_form';

					$el_value = $is_use_simgple_form;

					$params_checkbox = array(
											  'id'       => $el_id 		// HTML ID  of element
											, 'name'     => $el_id
											, 'label'    => array( 'title' => __('Simple' ,'booking') . ' ' . __('Booking Form', 'booking') , 'position' => 'right' )           //FixIn: 9.6.1.5
											...
											, 'selected' => ( 'On' === $is_use_simgple_form )		// Selected or not
											//, 'onfocus' =>  "console.log( 'ON FOCUS:',  jQuery( this ).is(':checked') , 'in element:' , jQuery( this ) );"					// JavaScript code
											, 'onchange' => "document.forms['{$submit_form_name}'].submit();"					// JavaScript code
											, 'hint' 	=> array( 'title' => __('Check the box, if you want to use simple booking form customization from Free plugin version at Settings - Form page.' ,'booking') , 'position' => 'top' )
										);
