...
function wpdev_bk_update_pay_status(){
	...
	// Record log
	wpbc_db__add_log_info( explode( ',', $booking_id ), 'Payment system response.' . ' -- ' . strtoupper( $pay_system ) . ' -- | ' . $status . ' |' );


    make_bk_action( 'wpbc_auto_approve_or_cancell_and_redirect', $pay_system, $status, $booking_id );
    
    // If the system was not redirecting yet, then redirect to home page - usually its has not happened.
    wpbc_redirect( site_url()  )   ;
}
