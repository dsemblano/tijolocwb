...
/**
 * @version 1.0
 * @package  iDEAL via Buckaroo (former Sisow)
...
//                                                                              <editor-fold   defaultstate="collapsed"   desc=" iDEAL    S u p p o r t   F u n c t i o n s " >    
    ...
    function wpbc_ideal_transaction_request() {
        ...
        $sysow_pay_options = $sisow_obj['sysow_pay_options'];
    ...
    function wpbc_ideal_get_sisow_obj( $params = array() ) {
        ...
        $sysow_pay_options = array();
        $sysow_pay_options[ 'is_active' ]             = get_bk_option( 'booking_ideal_is_active' );                // 'On' | 'Off'
                        
        if ( ! empty( $params ) ) {
            $sysow_pay_options[ 'subject' ]               = get_bk_option( 'booking_ideal_subject' );                  // 'Payment for booking %s on these day(s): %s'
                $sysow_pay_options[ 'subject' ] = apply_bk_filter('wpdev_check_for_active_language', $sysow_pay_options[ 'subject' ] );
                $sysow_pay_options[ 'subject' ] = wpbc_replace_booking_shortcodes( $sysow_pay_options[ 'subject' ], $params );
                $sysow_pay_options[ 'subject' ] = str_replace('"', '', $sysow_pay_options[ 'subject' ]);
                $sysow_pay_options[ 'subject' ] = substr($sysow_pay_options[ 'subject' ], 0, 32);
        }
        $sysow_pay_options[ 'return_url' ]            = get_bk_option( 'booking_ideal_return_url' );              // '/successful'
        $sysow_pay_options[ 'cancel_return_url' ]     = get_bk_option( 'booking_ideal_cancel_return_url' );       // '/failed'
        
        $sysow_pay_options[ 'payment_button_title' ]  = get_bk_option( 'booking_ideal_payment_button_title' );    // 'Pay via iDEAL'
            $sysow_pay_options[ 'payment_button_title' ]  =  apply_bk_filter('wpdev_check_for_active_language', $sysow_pay_options[ 'payment_button_title' ] );
                    
        $sysow_pay_options[ 'merchantid' ]   = get_bk_option( 'booking_ideal_merchant_id' );  //
        $sysow_pay_options[ 'merchantkey' ]  = get_bk_option( 'booking_ideal_merchant_key' ); //
        $sysow_pay_options[ 'shopid' ]       = '';
        $sysow_pay_options[ 'is_testmode' ]  = ( ( get_bk_option( 'booking_ideal_test' ) == 'TEST' ) ? true : false );
                
        //$sysow_pay_options[ 'curency' ]      = get_bk_option( 'booking_ideal_curency' );                  // 'EUR'
        $sysow_pay_options[ 'is_auto_approve_cancell_booking' ] = get_bk_option( 'booking_ideal_is_auto_approve_cancell_booking' );      // 'On' | 'Off'
        
        // New instance
        $sisow = new Sisow( $sysow_pay_options[ 'merchantid' ], $sysow_pay_options[ 'merchantkey' ], $sysow_pay_options[ 'shopid' ] );
    
        return array( 'sisow' => $sisow, 'sysow_pay_options' => $sysow_pay_options );
    }
//                                                                              </editor-fold>
...
class WPBC_Gateway_API_IDEAL extends WPBC_Gateway_API  {                     
    ...
    public function get_payment_form( $output, $params, $gateway_id = '' ) {
        ...
        $sisow = $sisow_obj['sisow'];
        $sisow_pay_options = $sisow_obj['sysow_pay_options'];            //FixIn: 9.8.14.1
	 ...
	 * We need to open payment form in separate window, if this booking was made togather with other
        ...
        */ 
        ?><form method="post" name="iDEAL_Payment<?php echo $html_client_id; ?>"><?php 
            ...
            $is_error = $sisow->DirectoryRequest( $return_banks, $is_selectbox, $sisow_pay_options[ 'is_testmode' ] );    // there are 2 methods for filling the available issuers in the select/dropdown below, the REST method DirectoryRequest is used
            if ( $is_error < 0 ) {
                // Some error
                if ( $is_error === -1 ) 
                    echo '<span class="wpdev-help-message wpdev-element-message alert alert-warning ">'
			...
			?><a href="javascript:void(0);" class="wpbc_button_light wpbc_button_gw wpbc_button_gw_ideal" onclick="javascript:wpbc_pay_via_ideal(  <?php echo $params['booking_resource_id']; ?> );"><?php echo $sisow_pay_options[ 'payment_button_title' ]; ?></a><?php
            ...
            ?><tr><td colspan="2" style="display:none;"><?php 
                ...
                // Description
                ?><input type="hidden" name="description" maxlength="32" value="<?php echo $sisow_pay_options[ 'subject' ]; ?>" /><?php
                                    ...
                                    , 'description' => __('Select TEST for the Test Server and LIVE in the live environment' ,'booking') 
                                                    . '<div class="wpbc-settings-notice notice-info" style="text-align:left;"><strong>' 
                                                        ...
                                                        . sprintf( __('Test mode requires the option %s to be selected in the %s account configuration section %s', 'booking' )
                                                                    ...
                                                                    , '<strong>Buckaroo (former Sisow)</strong>'
...
class WPBC_Settings_Page_Gateway_IDEAL extends WPBC_Page_Structure {
    ...
    public function tabs() {                                                    // T a b s      A r r a y
        ...
        $subtabs[ WPBC_IDEAL_GATEWAY_ID ] = array( 
                            ...
                            , 'title' =>  $icon .  'iDEAL Buckaroo'                     // Title of TAB
                            , 'page_title' => sprintf( __('%s Settings', 'booking'), 'iDEAL' )  // Title of Page   
                            , 'hint' => sprintf( __('Integration of %s payment system', 'booking'), 'iDEAL Buckaroo (former Sisow)' )
                            , 'link' => ''                                      // link
                            , 'position' => ''                                  // 'left'  ||  'right'  ||  ''
                    ...
                    <div class="wpbc-settings-notice notice-success" style="text-align:left;">
                        <?php 
                                printf( __('Processing your %s payments through %s' ,'booking')
                                        , '<strong>iDeal</strong>', '<strong><a href="https://www.sisow.nl" target="_blank">Buckaroo (former Sisow)</a></strong>' );
                        ?>
                    </div>
                    <div class="clear" style="height:10px;"></div>
<?php /**/ ?>                    
