...
class WPBC_Gateway_API_PAYPAL extends WPBC_Gateway_API  {                     
	 ...
	 * We need to open payment form in separate window, if this booking was made togather with other
        ...
        if ( $payment_options[ 'pro_hosted_solution' ] != 'On' ) {              // PayPal Standard
			...
			if ( ( ! empty( $payment_options['paypal_tax_fee'] ) ) && ( intval( $payment_options['paypal_tax_fee'] ) > 0 ) ) {
				 ?><input type="hidden" name="tax" value="<?php echo ( ( intval( $payment_options['paypal_tax_fee'] ) / 100 ) * floatval( $params['cost_in_gateway'] ) ); ?>" /><?php			// Tax
			}
	...
	if(0)
        ...
        if ( $payment_options[ 'button_type' ] != 'paypal_custom_text_v1' ) {
            ?><input type="submit" class="wpbc_button_light wpbc_button_gw wpbc_button_gw_paypal wpbc_button_gw_paypal_yellow" name="submit"  value="<?php echo $payment_options[ 'payment_button_title' ]; ?>" /><?php
        } else {
			?><input type="submit" class="wpbc_button_light wpbc_button_gw wpbc_button_gw_paypal" name="submit"  value="<?php echo $payment_options[ 'payment_button_title' ]; ?>" /><?php
        ...
        if ( ( ! empty( $payment_options['paypal_tax_fee'] ) ) && ( intval( $payment_options['paypal_tax_fee'] ) > 0 ) ) {
	        echo "<div class='wpbc_paypal_fee'> ("
				 	  ...
				 	  . wpbc_cost_show(
				 	  					 ( ( intval( $payment_options['paypal_tax_fee'] ) / 100 ) * floatval( $params['cost_in_gateway'] ) )
...
//                                                       . '<br/>' . sprintf( __('PayPal sandbox can be used to test payments. Sign up for a developer account %shere%s.', 'booking'), '<a href="https://developer.paypal.com/" targe="_blank">', '</a>' )
        ...
        // Payment Button        
        $field_options = array(
                                'paypal_yellow_v1' => array(  'title' => '<span class="wpbc_container" id="paypal_button_type_1_custom_yellow"><span class="wpbc_button_light wpbc_button_gw wpbc_button_gw_paypal wpbc_button_gw_paypal_yellow"></span></span>'//'<img src="'. WPBC_PAYPAL_PAY_BUTTON_URL .'" style="margin: 0;"/>'
                                                            , 'attr' => array( 'id' => 'paypal_button_type_1' )                                                             
                                                            , 'html' => '<br/>')
                              , 'paypal_custom_text_v1' => array(  'title' => '<span class="wpbc_container" id="paypal_button_type_4_custom_standard"><span class="wpbc_button_light wpbc_button_gw wpbc_button_gw_paypal">'
                                                               . apply_bk_filter( 'wpdev_check_for_active_language', get_bk_option( 'booking_paypal_payment_button_title' ) ) // __('Custom button title' ,'booking')
															   .'</span></span>'
...
//            );             
    ...
    public function get_gateway_info() {
        ...
        $gateway_info = array(
                      'id'       => $this->get_id()
        			, 'title' => ( ( $type != 'On' ) ? __( 'Paypal Standard', 'booking' ) : __( 'Paypal Pro Hosted Solution', 'booking' ) ) .  ' (' . __( 'Legacy', 'booking' ) . ')'
...
class WPBC_Settings_Page_Gateway_PAYPAL extends WPBC_Page_Structure {
    ...
    private function check_compatibility_with_older_7_ver() {
         ...
         * So  if its does not standar  button,  then  set 'paypal_yellow_v1' value for definition PayPal Img Button.
         * The link to  this button  defined at the CONSTANT at  top  of this file
         */
        
        $field_value = get_bk_option( 'booking_paypal_button_type' );
        
        if (  ( $field_value != 'paypal_custom_text_v1' ) && (  $field_value != 'paypal_yellow_v1' )  ){
            update_bk_option( 'booking_paypal_button_type', 'paypal_yellow_v1' );
        }
    }
    ...
    public function tabs() {                                                    // T a b s      A r r a y
        ...
        $subtabs[ WPBC_PAYPAL_GATEWAY_ID ] = array( 
                            ...
                            , 'title' =>  $icon . __('PayPal' ,'booking') . ' (' . __( 'Legacy', 'booking' ) . ')'		// Title of TAB
    ...
    private function enqueue_js(){                                                        
		...
		$js_script .= "  jQuery( document ).on( 'keyup', '#paypal_payment_button_title', function (){
							jQuery( '#paypal_button_type_4_custom_standard span' ).html( jQuery( '#paypal_payment_button_title' ).val() );
		                  } );
...
function wpbc_booking_activate_PAYPAL() {
    ...
    add_bk_option( $op_prefix . 'button_type', 'paypal_yellow_v1' );
