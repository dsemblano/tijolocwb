...
class WPBC_Page_SettingsGateways_duplicate extends WPBC_Page_Structure {
    ...
    public function tabs() {
        ...
        $tabs = array();
        $tabs[ 'payment' ] = array(
                              'title'       => __('Payment Gateways','booking')  . '<span class="wpbc_new_label" style="float:none;padding:0 0 0 2em;">New</span>'                   // Title of TAB
                            , 'hint'        => __('Customization of Payment', 'booking')     // Hint
                            , 'page_title'  => __('Payment Gateways', 'booking')            // Title of Page
                            , 'link' =>  wpbc_get_settings_url( true, false ) . '&tab=payment'                                      // Can be skiped,  then generated link based on Page and Tab tags. Or can  be extenral link
                            , 'position' => ''                                  // 'left'  ||  'right'  ||  ''
                            , 'css_classes' => 'wpbc_top_tab__payment_gateways'   // CSS class(es)
...
add_action('wpbc_menu_created', array( new WPBC_Page_SettingsGateways_duplicate() , '__construct') );    // Executed after creation of Menu
...
function wpbc_get_default_options_gw() {
	...
	$default_options[ 'booking_payment_description'] = '<p>Dear <strong>[name]</strong>, your booking ID #<strong>[booking_id]</strong> for the <strong>[resource_title]</strong> on <strong>[selected_short_dates_hint]</strong>, requires your payment for confirmation.</p>';
    $default_options[ 'booking_currency' ]       = 'USD';
    $default_options[ 'booking_currency_pos' ]   = 'left';
	//FixIn: 8.4.7.20
    $default_options[ 'booking_gateways_order' ] = 'stripe_v3,paypal_std_co,paypal,authorizenet,sage,bank_transfer,pay_cash,ipay88,ideal';    // ,ideal		//FixIn: 8.6.1.12		//FixIn: 9.8.14.5
 ...
 */
function wpbc_get_gateway_forms( $params ) {
    ...
    // -----------------------------------------------------------------------------------------------------------------
    // Replace Params 		-		Get different parameters for replacing in payment forms (like in Pay by  cash  form)
    // -----------------------------------------------------------------------------------------------------------------
    $replace_params = wpbc_get_booking_different_params_arr( $params['booking_id'], $params['form'], $params['resource_id'] );  		// [ ... [dates_count] => 2, [cost] => 20000.00, ... ]

    foreach ( $params as $params_key => $params_value ) {
																							// Skip some input parameters, because they already  exist.
	    if ( ! in_array( $params_key, array( 'cost', 'booking_id', 'resource_id' ) ) ) {
    ...
    if ( class_exists( 'wpdev_bk_biz_m' ) ) {
        ...
        $total_cost_of_booking = wpbc_get_costs_i_hints__based_on_cost_calc( 
                                                            array(
                                                                    ...
                                                                    , 'is_check_additional_calendars' => false  // We do  not check  about additional  calendars, ust  calculate cost for specific 1 calendar. Because,  we will show payment forms for each  such specific calendar separately
                                                                ) 
                                                        );
    } else {                                                                    // Blank data for other versions.
	...
	//   TODO: If we are having 		'ADDITIONAL CALENDARS' 		in the booking form  then use this (to  open payment forms in separate TABs)
	//   $replace_params['payment_form_target'] = ' target="_blank" ';

    
    // Replace currency symbol to CURRENCY_SYMBOL
    ...
    $payment_varriants[] = $replace_params;					// Deposit or Total or Balance
	$payment_varriants[ ( count( $payment_varriants ) - 1 ) ]['cost_in_gateway'] = $params['cost'];
	$payment_varriants[ ( count( $payment_varriants ) - 1 ) ]['is_deposit'] 	 = $params['is_deposit'];
	...
	// If needed, then get only selected payment form
	if ( ! empty( $replace_params['payment-method'] ) ) {
		$gateways_order = wpbc_filter_payment_gateway_arr__to__selected_payment_gateway( $gateways_order, $replace_params['payment-method'] );
	}
    ...
    ////////////////////////////////////////////////////////////////////////////
	$output_arr[ 'gateways_arr' ] = array();
    foreach ( $payment_varriants as $payment_varriant ) {                       // Despoit or Total ...
            
        foreach ( $gateways_order as $gateway_id) {                                                 // Order of Gateways ...

			// Here Go -------------------------------------------------------------------------------------------------
 	        $current_payment_output = apply_filters( 'wpbc_get_gateway_payment_form', '', $payment_varriant, $gateway_id );

	        if ( ! empty( $current_payment_output ) ) {

				// ? -> Now, we do this in template and not via innerHTML. PREVIOUSLY: We are needed to replace ' symbols because in usual payment form, we assign this output to JavaScript string: ....innerHTML = '<div class=\"wpdevelop\" style=\"height:auto;margin:20px 0px;\" >?php echo $output; ?</div>';
		        $current_payment_output = str_replace( array( "'", '\"' ), '"', $current_payment_output );

				$gateway_params_arr = array(	//'payment_params' => $payment_varriant,
												'gateway_id'     => $gateway_id,
												'output'         => $current_payment_output
											);
				// Unset  some secure parameters to not transfer to  client side
		        if ( ( isset( $gateway_params_arr['payment_params'] ) ) && ( isset( $gateway_params_arr['payment_params']['__nonce'] ) ) ) {
					unset( $gateway_params_arr['payment_params']['__nonce'] );
				}
				$output_arr['gateways_arr'][] = $gateway_params_arr;
			}
        }        
    }

	$output_arr['gateway_cost']   = $params['cost'];
	$output_arr['is_for_deposit'] = ( ( empty( $params['is_deposit'] ) ) ? 0 : 1 );


	if ( 'On' != get_bk_option( 'booking_show_deposit_and_total_payment' ) ) {

		$output_arr['header'] = ( $output_arr['is_for_deposit'] )
								? __( 'Pay Deposit cost', 'booking' )
								: __( 'Amount to pay', 'booking' );
	} else {
		$output_arr['header'] = ( $output_arr['is_for_deposit'] )
								? __( 'Pay Deposit Only', 'booking' )
								: __( 'Pay Total Cost', 'booking' );
	}
	$output_arr['header'] .= ': ' . wpbc_get_cost_with_currency_for_user( $params['cost'], $params['resource_id'] );


	// PAYMENT DESCRIPTION
    if ( 'On' == get_bk_option( 'booking_is_show_booking_summary_in_payment_form'  ) ) {
	    ...
	    $output_arr['booking_summary'] = $payment_description_show;
    } else {
	    $output_arr['booking_summary'] = '';
	}
...
function wpbc_load_payment_gateways_files() {
    ...
    //FixIn: 8.4.7.20
    $booking_gateways_original = 'stripe_v3,paypal_std_co,paypal,authorizenet,sage,bank_transfer,pay_cash,ipay88,ideal';   //     // Default Original Payment Gateways	//FixIn: 8.6.1.12		//FixIn: 9.8.14.5
...
// TODO: Set  target="_blank" to the payment form  if we are showing payment forms for several  additional  calendars
	...
	/**
	 * If user was selected (in booking form) the  specific payment form  to  show,  then  filter all  payment systems and get  only  selected
	 *
	 * @param array $gateways_order
	 * @param atring $payment_method
	 *
	 * @return mixed
	 */
	function wpbc_filter_payment_gateway_arr__to__selected_payment_gateway( $gateways_order, $payment_method = '' ) {
		...
		if ( ! empty( $payment_method ) ) {
			...
			//FixIn: 8.6.1.16 	-	Rechecking for situation of usage only labels: [select payment-method "All payment methods@@" "Stripe" "PayPal" "Authorize.Net" "Sage Pay" "Bank Transfer" "Pay in Cash" "iPay88" "iDEAL"]
			$all_payment_methods = array(
									"Stripe"                   => "stripe_v3",
									"PayPal Standard Checkout" => "paypal_std_co",                //FixIn: 9.8.14.5
									"PayPal"                   => "paypal",
			...
			$selected_gateway = $payment_method;

			// Rechecking if this payment method exist  in the supported payment systems
			if ( 	   ( in_array( $payment_method, $gateways_order ) )
