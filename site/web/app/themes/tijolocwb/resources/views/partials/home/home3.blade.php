<section id="tela3" class="home-section h-screen bg-tijoloyellow">
  <div class="container flex flex-col justify-center items-center h-full">
    {{-- {!! do_shortcode('[wpgmza id="1"]') !!} --}}
    {{-- {!! do_shortcode('[rank_math_contact_info class="w-full lg:w-3/5" show="map"]') !!} --}}
    @include('partials/snippets.gmaps')
    <h6 class="text-lg xl:text-2xl text-gray-800 top-10">Rua São Francisco 179 loja 3,<br />Curitiba PR CEP 80020-190</h6>
    <br>
  </div>
</section>
